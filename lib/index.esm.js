import React, { forwardRef, useState, useRef, useMemo, useCallback, useEffect, useImperativeHandle, useLayoutEffect } from 'react';
...
var styles = {"slot_wrap":"index-module_slot_wrap__ZT-DX","slot":"index-module_slot__DpPgW","separator":"index-module_separator__7GhtE"};
...
function SlotCounter(_a, ref) {
    var value = _a.value, _b = _a.duration, duration = _b === void 0 ? 0.6 : _b, charClassName = _a.charClassName, separatorClassName = _a.separatorClassName;
    var _c = useState(false), active = _c[0], setActive = _c[1];
    var _d = useState(value), localValue = _d[0], setLocalValue = _d[1];
    var _e = useState(0), fontHeight = _e[0], setFontHeight = _e[1];
    var numbersRef = useRef(null);
    var dummyNumbers = useMemo(function () { return range(1, DUMMY_NUMBER_COUNT).map(function () { return random(1, 10); }); }, []);
    ...
    useEffect(function () {
        reloadAnimation();
        setTimeout(function () { return setLocalValue(value); }, value.toString().length * 100);
    ...
    return (React.createElement("div", { className: mergeClassNames(styles.slot_wrap, active && styles.active) }, localValue
        ...
        if (SEPARATOR.includes(v)) {
            return (React.createElement("div", { key: i, className: mergeClassNames(styles.separator, separatorClassName) }, v));
        }
        return (React.createElement("div", { key: i, className: mergeClassNames(styles.slot, charClassName), style: { height: fontHeight } },
            React.createElement("div", { ref: numbersRef, className: styles.numbers, style: __assign({ transition: 'none' }, (active && {
                ...
                dummyNumbers.map(function (dummyNumber, slotIndex) { return (React.createElement("div", { key: slotIndex, className: styles.num }, dummyNumber)); }),
                React.createElement("div", { className: styles.num }, v))));
    })));
}
